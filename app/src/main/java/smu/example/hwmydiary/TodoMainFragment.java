...
public class TodoMainFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        // fragment_main에 인플레이션
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.todofragment_main, container, false);
        initUI(rootView);
        loadNoteListData();
    ...
    private void initUI(ViewGroup rootView){
        ...
        // 어댑터 연결
        adapter = new TodoNoteAdapter();
        recyclerView.setAdapter(adapter);
    }

    public int loadNoteListData(){

        // 데이터를 가져오는 sql문 select
        String loadSql = "select _id, TODO from " + TodoNoteDatabase.TABLE_NOTE + " order by _id desc";
        ...
        if(database != null){
            ...
            // _id, TODO가 담겨질 배열 생성
            ArrayList<TodoNote> items = new ArrayList<>();
            ...
            for(int i = 0; i < recordCount; i++){
                ...
                items.add(new TodoNote(_id,todo));
            }
            outCursor.close();
