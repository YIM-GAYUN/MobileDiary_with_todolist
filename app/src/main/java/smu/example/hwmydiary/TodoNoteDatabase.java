...
public class TodoNoteDatabase {
    ...
    private static TodoNoteDatabase database;
    ...
    public static TodoNoteDatabase getInstance(Context context){
        if(database == null){
            database = new TodoNoteDatabase(context);
        }
        return database;
    }

    // 이후 리스트를 표시할 때 위치를 나타내는 커서역할을 함
